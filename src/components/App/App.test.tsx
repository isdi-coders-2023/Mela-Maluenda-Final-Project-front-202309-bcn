...
describe("Given the App component", () => {
  ...
  describe("When it is rendered on the ModifyPage and the user clicks on the button: 'Modify'", () => {
    test("Then it should modify the photo and go back to HomePage", async () => {
  ...
  describe("When is rendered and the user clicks the photo card 'Caravaggio routines'", () => {
    test("Then it should show a the description of what made you click", async () => {
      const expectedTextLink = "« Caravaggio routines »";
      ...
      const heading = screen.getByRole("heading", {
        name: expectedTextLink,
      });

      await userEvent.click(heading);

      const whatMadeYouClick = await screen.findByText(expectedDescription);

      expect(whatMadeYouClick).toBeInTheDocument();
    });
  });
});
